...
app.openapi(SponsoredInfoRoute, async (c) => {
  const networkMode = parseNetworkMode(c.req.header('x-network-mode'))
...
app.openapi(RelayInfoRoute, async (c) => {
  ...
  if (!contractAddress || networkMode === 'mainnet') return c.json({ error: 'Relay not configured for chain' }, 404)
...
app.openapi(GasPoolRoute, async (c) => {
  ...
  if (isNaN(chainId)) return c.json({ error: 'Invalid chainId' }, 400 as any)
  const networkMode = parseNetworkMode(c.req.header('x-network-mode'))
  const pools = await getGasPool(chainId, networkMode)
